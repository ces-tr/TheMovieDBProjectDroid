...
public abstract class OverScrollBounceEffectDecoratorBase implements IOverScrollDecor, View.OnTouchListener {
    ...
    private final boolean disablePullDownEffect;
    ...
    protected class OverScrollingState implements IDecoratorState {
        ...
        public boolean handleMoveTouchEvent(MotionEvent event) {
            ...
            if (!disablePullDownEffect || newOffset < 0) {
    ...
    public OverScrollBounceEffectDecoratorBase(IOverScrollDecoratorAdapter viewAdapter, float decelerateFactor, float touchDragRatioFwd, float touchDragRatioBck) {
        ...
        disablePullDownEffect = isSwipeToRefreshEnabled(mViewAdapter);

        attach();
    }

    private boolean isSwipeToRefreshEnabled(IOverScrollDecoratorAdapter adapter) {

        if (adapter instanceof NestedScrollViewOverScrollDecorAdapter) {
            return ((NestedScrollViewOverScrollDecorAdapter) adapter).swipeToRefreshEnabled();
        }
        else if(adapter instanceof RecyclerViewOverScrollDecorAdapter){

            return ((RecyclerViewOverScrollDecorAdapter) adapter).swipeToRefreshEnabled();
        }
